...
class TestICDGenerator:
    ...
    def __init__(self):
        ...
        self.bitfield_patterns = [
            ...
            {'mask': 0x0002, 'shift': 1},   # Bit 1
            {'mask': 0x0004, 'shift': 2},   # Bit 2
            {'mask': 0x0008, 'shift': 3},   # Bit 3
            {'mask': 0x0010, 'shift': 4},   # Bit 4
            {'mask': 0x0020, 'shift': 5},   # Bit 5
            {'mask': 0x0040, 'shift': 6},   # Bit 6
            {'mask': 0x0080, 'shift': 7},   # Bit 7
            {'mask': 0x0100, 'shift': 8},   # Bit 8
            {'mask': 0x0200, 'shift': 9},   # Bit 9
            {'mask': 0x0400, 'shift': 10},  # Bit 10
            {'mask': 0x0800, 'shift': 11},  # Bit 11
            {'mask': 0x1000, 'shift': 12},  # Bit 12
            {'mask': 0x2000, 'shift': 13},  # Bit 13
            {'mask': 0x4000, 'shift': 14},  # Bit 14
            {'mask': 0x8000, 'shift': 15},  # Bit 15
            ...
            # Shifted multi-bit fields
            {'mask': 0x00F0, 'shift': 4},   # 4 bits at position 4
            {'mask': 0x0F00, 'shift': 8},   # 4 bits at position 8
            {'mask': 0xF000, 'shift': 12},  # 4 bits at position 12
            {'mask': 0x03C0, 'shift': 6},   # 4 bits at position 6
            {'mask': 0x1F00, 'shift': 8},   # 5 bits at position 8
            {'mask': 0x7C00, 'shift': 10},  # 5 bits at position 10
        ...
        for field_idx in range(num_fields):
            ...
            for pattern in available_patterns:
                ...
                if (used_bits & mask) == 0:
                    ...
                    # Add units for some fields
                    if random.random() > 0.6:
                        field['units'] = random.choice(['deg', 'ft', 'm', 'kts', 'g', 'rad', 'Hz', 'V', 'A'])
    ...
    def generate_message(self, msg_num: int, sub_address: int, num_words: int = None) -> Dict[str, Any]:
        ...
        message = {
            'name': f'TEST_MSG_{msg_num:04d}',
            'sub_address': sub_address,
            'description': f'Test message {msg_num} with {num_words} words',
            'rate': random.choice([1, 5, 10, 20, 50, 100]),  # Hz
            'words': []
        }
    ...
    def generate_icd(self, num_messages: int, name: str = None) -> Dict[str, Any]:
        ...
        # Use sub-addresses 1-30
        sub_addresses = list(range(1, 31))
        
        for msg_num in range(num_messages):
            sub_address = random.choice(sub_addresses)
            message = self.generate_message(msg_num, sub_address)
