...
def encode_bitfield(value: Union[int, float], mask: int, shift: int, 
    ...
    if not (0 <= mask <= 0xFFFF):
        raise ValueError(f"Mask must be 0-65535, got {mask}")
    if not (0 <= shift <= 15):
        raise ValueError(f"Shift must be 0-15, got {shift}")
    ...
    if mask != 0:
        ...
        if highest_bit + shift > 16:
            raise ValueError(f"Mask 0x{mask:04X} with shift {shift} exceeds 16 bits")
...
def pack_bitfields(fields: Dict[str, Tuple[Union[int, float], int, int, float, float]]) -> int:
    ...
    for field_name, (value, mask, shift, scale, offset) in fields.items():
        ...
        if used_bits & shifted_mask:
            raise ValueError(f"Field '{field_name}' overlaps with previously packed fields")
...
def build_command_word(rt: int, tr: bool, sa: int, wc: int) -> int:
    ...
    """
    if rt < 0 or rt > 31:
        raise ValueError(f"RT must be 0-31, got {rt}")
    if sa < 0 or sa > 31:
        raise ValueError(f"SA must be 0-31, got {sa}")
    if wc < 1 or wc > 32:
        raise ValueError(f"WC must be 1-32, got {wc}")
