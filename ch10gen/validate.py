            ...
            for msg in packet:
                ...
                if hasattr(msg, 'data') and msg.data:
                    ...
                    if len(msg.data) > 0:
                        ...
                        # Check data word count (should be command + status + data words)
                        expected_total = 2 + wc  # Command word + status word + data
                        if len(msg.data) != expected_total:
                            self.stats['warnings'].append(
                                f"Data word count mismatch: expected {expected_total}, got {len(msg.data)}"
                            )
