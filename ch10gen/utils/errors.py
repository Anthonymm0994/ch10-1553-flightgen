...
class MessageErrorInjector:
    ...
    def inject_errors(self, message_time_s: float, command_word: int, 
                     status_word: int, data_words: List[int]) -> Tuple[int, int, List[int], ErrorType]:
        """
        Inject errors into a message.
        
        Args:
            message_time_s: Message timestamp in seconds
            command_word: Original command word
            status_word: Original status word
            data_words: Original data words
