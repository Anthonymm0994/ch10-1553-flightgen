...
## XML to YAML Converter Updates

The XML converter needs these fixes:

1. **Word number at top**: Move `word:` to be first field in word definition
2. **Default bus B**: Change from 'A' to 'B'
3. **Use filename**: Use input filename (without extension) as ICD name
4. **No src field**: Don't add `src` attributes to fields

Example corrected output:
```yaml
name: my_icd_file  # From my_icd_file.xml
bus: B             # Default to B
messages:
  - name: "Navigation Data"
    sub_address: 1
    words:
      - word: 0    # Word number at top
        fields:
          - name: altitude
            mask: 0x3FFF
            shift: 0
