...
class TestCommandWord:
    ...
    def test_invalid_parameters(self):
        ...
        with pytest.raises(ValueError, match="RT must be 0-31"):
            build_command_word(rt=32, tr=True, sa=1, wc=16)
        
        with pytest.raises(ValueError, match="SA must be 0-31"):
            build_command_word(rt=1, tr=True, sa=32, wc=16)
        
        with pytest.raises(ValueError, match="WC must be 1-32"):
